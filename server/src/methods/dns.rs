...
/// Thanks to: <https://github.com/EmilHernvall/dnsguide>
pub fn dns_exfiltrator(
    dns_ip: &String,
    dns_port: &String,
    key: &String,
) -> Result<()> {
    info!("Starting fake DNS server on {}",format!("{}:{}",dns_ip,dns_port).to_string().bold().green());

    let mut id_hexadata = HashMap::new();
    let socket = UdpSocket::bind(format!("{}:{}",dns_ip,dns_port))?;
    loop {
        match handle_query(
            &socket,
            &mut id_hexadata,
